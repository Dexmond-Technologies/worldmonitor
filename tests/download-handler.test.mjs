...
const RELEASES_PAGE = 'https://github.com/Dexmond-Technologies/WORLD_MONITOR/releases/latest';
...
test('matches full variant for dotted World.Monitor AppImage asset names', async () => {
  ...
  try {
    const response = await handler(
      new Request('https://WORLD_MONITOR.app/api/download?platform=linux-appimage&variant=full')
    );
...
test('matches tech variant for dashed Tech-Monitor AppImage asset names', async () => {
  ...
  try {
    const response = await handler(
      new Request('https://WORLD_MONITOR.app/api/download?platform=linux-appimage&variant=tech')
    );
    assert.equal(response.status, 302);
    assert.equal(
      response.headers.get('location'),
...
test('falls back to release page when requested variant has no matching asset', async () => {
  ...
  try {
    const response = await handler(
      new Request('https://WORLD_MONITOR.app/api/download?platform=linux-appimage&variant=finance')
    );
    assert.equal(response.status, 302);
